...
class Customer {
  ...
  constructor(name: string, email: string, mobNo: number, accountNo: number) {
    this.name = name;
    this.email = email;
    this.mobNo = mobNo;
...
class Bank {
  ...
  addAccount(accountObj: Account) {
    this.accounts.push(accountObj);
  }

  DepositeAmount(amountObj: Account) {
    let index = this.accounts.findIndex(
      (item) => item.accountNo == amountObj.accountNo
    );
...
async function authenticate() {
  ...
  currentUser = bank.customers.find(
    (item: Customer) => item.accountNo === authentication.accNo
  )!;
  if (currentUser) {
...
async function bankOperations(bank: Bank) {
  ...
  let userAccount = bank.accounts.find(
    (item) => item.accountNo == currentUser.accountNo
  );
  ...
  if (operations.operation === "Check Balance") {
    console.log(
      `${currentUser.name} your balance is PKR:${userAccount?.balance}`
    );
    return playAgain();
  }
